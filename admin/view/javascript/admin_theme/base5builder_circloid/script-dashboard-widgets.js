$(document).ready(function(){
	...
	*/
	...
	function getOrderCustomerChart(range, canvasID) {
		...
		var orderStatus = $('#order-status-customer').val();

		if(range == "custom"){

			var rangeCustomStart = $('#range-custom-start').val();
			var rangeCustomEnd = $('#range-custom-end').val();

			$('.range-custom-input').datepicker({
				format: 'dd-mm-yyyy'
			});
		}else{

			// Give empty date range for non-custom range
			var rangeCustomStart = 0;
			var rangeCustomEnd = 0;
		}
		...
		$.ajax({
			type: 'GET',
			url: 'index.php?route=common/home/chart&token=' + split[1][1] + '&range=' + range + '&range_custom_start=' + rangeCustomStart + '&range_custom_end=' + rangeCustomEnd + '&chart_order_status=' + orderStatus,
			dataType: 'json',
			beforeSend: function(){
			...
			success: function(json) {
				...
				if(canvasID == "#order-customer-chart"){
					if(range == "custom"){
						pointRadius = 0;
				...
				var option = {	
					...
					tooltipOpts: {
						content: function(label, xval, yval, flotItem){
							if(canvasID == "#order-customer-chart" && range == "custom"){
	...
	if(chartOrderCustomerExists > 0){
		...
		$("#range-order-customer").change(function(){
			...
			if(chartRange == "custom"){
				$(".custom-range-fields").fadeIn(200);
			}else{
				$(".custom-range-fields").fadeOut(200);
			}

		});

		$(".custom-range-fields button").click(function(){
			// Get Date Range
			var chartRange = $('#range-order-customer').val();
			// Plot It
			getOrderCustomerChart(chartRange, "#order-customer-chart");
		...
		$("#order-status-customer").change(function(){
			// Get Date Range
			var chartRange = $('#range-order-customer').val();
			// Plot It
			getOrderCustomerChart(chartRange, "#order-customer-chart");
