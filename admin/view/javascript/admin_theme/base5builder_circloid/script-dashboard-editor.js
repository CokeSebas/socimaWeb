$(document).ready(function(){
	...
	*/
	...
	/* Circloid Dashboard Settings - Start */


	function saveCircloidSettings() {

		// Get Setting Options
		
		var outOfStock = encodeURIComponent($("#out-stock-control").val());
		var lowStock = encodeURIComponent($("#low-stock-control").val());
		var lowStockCount = encodeURIComponent($("#low-stock-count").val());
		var menuEventType = encodeURIComponent($("#menu-event-type-control").val());
		...
		$.ajax({
			type: 'GET',
			url: 'index.php?route=common/admin_circloid_dashboard_editor/saveCircloidSettings&token=' + split[1][1] + '&out_stock=' + outOfStock + '&low_stock=' + lowStock + '&low_stock_count=' + lowStockCount + '&menu_event_type=' + menuEventType,
			dataType: 'json',
			beforeSend: function(){
				$(".dashboard-editor-setting .loading").show();
			},
			error: function(){
				...
				$(".dashboard-editor-setting .loading").hide();
			},
			success: function(json) {
				...
				if(json.success_saved_settings){
					$('<div class="success">' + json.success_saved_settings + '</div>').appendTo(".dashboard-editor-content").css("visibility", "visible").fadeIn(300, function(){
						setTimeout(function(){
					...
					$('<div class="warning">' + json.error_saved_settings + '</div>').appendTo(".dashboard-editor-content").css("visibility", "visible").fadeIn(300, function(){
						setTimeout(function(){
							$(".success").fadeOut(300, function(){
								$(this).remove();
							});
	...
	$("#settings-save").click(function(e){
		saveCircloidSettings();
		e.preventDefault();
	});
	/* Circloid Dashboard Settings - End */
