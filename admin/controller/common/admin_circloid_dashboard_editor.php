...
class ControllerCommonAdminCircloidDashboardEditor extends Controller {
	...
	public function index(){
		...
		$this->data['heading_circloid_settings'] = $this->language->get('heading_circloid_settings');
		...
		$this->data['text_low_stock_count'] = $this->language->get('text_low_stock_count');
		$this->data['text_out_stock'] = $this->language->get('text_out_stock');
		$this->data['text_menu_event_type'] = $this->language->get('text_menu_event_type');
		...
		$this->data['text_installation_info'] = $this->language->get('text_installation_info');
		...
		// Get Circloid Settings
		$this->data['circloid_settings'] = $this->model_common_admin_circloid_dashboard_editor->getCircloidSettings();
		$this->data['install_circloid_settings'] = $this->url->link('common/admin_circloid_dashboard_editor/installCircloidSettings', 'token=' . $this->session->data['token'], 'SSL');
			...
			foreach ($xml_file as $xml) {
				if($xml->attributes()->name == "admin/view/template/common/header.tpl"){
					...
					foreach($xml->operation as $operation_block){
						$xml_rebuild .= '		<operation error="skip">';
						$xml_rebuild .= "\n";
	...
	public function installCircloidSettings(){
		...
		if(!$this->user->hasPermission('modify', 'common/admin_circloid_dashboard_editor')){
			...
			$this->data['success_circloid_settings_installed'] = $this->language->get('success_circloid_settings_installed');
			$this->data['error_circloid_settings_installed'] = $this->language->get('error_circloid_settings_installed');

			// Call the model
			$this->load->model('common/admin_circloid_dashboard_editor');

			$circloid_settings_installed = $this->model_common_admin_circloid_dashboard_editor->installCircloidSettings();

			if($circloid_settings_installed){
				$this->session->data['success'] = $this->language->get('success_circloid_settings_installed');
				...
				$this->session->data['error'] = $this->language->get('error_circloid_settings_installed');
		...
		$this->redirect($this->url->link('common/admin_circloid_dashboard_editor', 'token=' . $this->session->data['token'], 'SSL'));
	}

	public function saveCircloidSettings(){

		$this->language->load('common/home');

		if(!$this->user->hasPermission('modify', 'common/admin_circloid_dashboard_editor')){
			$data['error_saved_settings'] = $this->language->get('error_permission');
		}else{

			$data['out_stock'] = urldecode($this->request->get['out_stock']);
			$data['low_stock'] = urldecode($this->request->get['low_stock']);
			$data['low_stock_count'] = urldecode($this->request->get['low_stock_count']);
			$data['menu_event_type'] = urldecode($this->request->get['menu_event_type']);

			// Call the model
			$this->load->model('common/admin_circloid_dashboard_editor');

			// Send to model
			$data['settings_saved'] = $this->model_common_admin_circloid_dashboard_editor->saveCircloidSettings($data);

			$data['success_saved_settings'] = $this->language->get('success_saved_settings');
		}
