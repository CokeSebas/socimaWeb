...
class ModelSaleCustomer extends Model {
	public function addCustomer($data) {
		$CreditoActual = str_replace(".","",$data['creditoActual']);
		$CreditoMaximo = str_replace(".","",$data['creditoMaximo']);
	
		$this->db->query("INSERT INTO " . DB_PREFIX . "customer SET firstname = '" . $this->db->escape($data['firstname']) . "', lastname = '" . $this->db->escape($data['lastname']) . "', email = '" . $this->db->escape($data['email']) . "', telephone = '" . $this->db->escape($data['telephone']) . "', fax = '" . $this->db->escape($data['fax']) . "', newsletter = '" . (int)$data['newsletter'] . "', customer_group_id = '" . (int)$data['customer_group_id'] . "', salt = '" . $this->db->escape($salt = substr(md5(uniqid(rand(), true)), 0, 9)) . "', password = '" . $this->db->escape(sha1($salt . sha1($salt . sha1($data['password'])))) . "', status = '" . (int)$data['status'] . "', Credito = '" . (int)$CreditoActual . "', CreditoMaximo = '" . (int)$CreditoMaximo . "', salesrep_id = '" . (int)$data['vendedor'] . "', date_added = NOW()");
	...
	public function editCustomer($customer_id, $data) {
		...
		$this->db->query("UPDATE " . DB_PREFIX . "customer SET firstname = '" . $this->db->escape($data['firstname']) . "', lastname = '" . $this->db->escape($data['lastname']) . "', email = '" . $this->db->escape($data['email']) . "', telephone = '" . $this->db->escape($data['telephone']) . "', fax = '" . $this->db->escape($data['fax']) . "', newsletter = '" . (int)$data['newsletter'] . "', customer_group_id = '" . (int)$data['customer_group_id'] . "', status = '" . (int)$data['status'] . "', Credito = '" . (int)$CreditoActual . "', CreditoMaximo = '" . (int)$CreditoMaximo . "', salesrep_id = '" . (int)$data['vendedor'] . "' WHERE customer_id = '" . (int)$customer_id . "'");
	...
	public function getSalesRep(){
		$query = $this->db->query("SELECT salesrep_id, name FROM " . DB_PREFIX . "salesrep");
	...
	public function getCustomers($data = array()) {
		...
		//var_dump($groups['user_group_id']);
		
		if(($groups['user_group_id'] != 1) AND ($groups['user_group_id'] != 16)){
			...
			$sql = "SELECT *, CONCAT(c.firstname, ' ', c.lastname) AS name, cgd.name AS customer_group FROM " . DB_PREFIX . "customer c JOIN " . DB_PREFIX . "customer_group_description cgd ON (c.customer_group_id = cgd.customer_group_id) WHERE cgd.language_id = '" . (int)$this->config->get('config_language_id') . "'";
			...
			$sql = str_replace (' JOIN ', " JOIN " . DB_PREFIX . "salesrep sr ON (c.salesrep_id = sr.salesrep_id) JOIN ", $sql);
			// madimar mod end	
	...
	public function getOrders($data = array()) {
		...
		if (isset($data['filter_order_status_id']) && !is_null($data['filter_order_status_id'])) {
			$sql .= " WHERE o.order_status_id = '" . (int)$data['filter_order_status_id'] . "' AND customer_id = '" . $data['customerid'] . "'";
		} else {
			$sql .= " WHERE o.order_status_id > '0' AND customer_id = '" . $data['customerid'] . "'";
		}
