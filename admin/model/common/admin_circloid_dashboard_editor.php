...
class ModelCommonAdminCircloidDashboardEditor extends Model {
	...
	public function tablesExist(){
		...
		$query = $this->db->query("SHOW TABLES LIKE '" . DB_PREFIX . "circloid_settings'");

		if($query->num_rows > 0){
			$data['circloid_setting'] = 1;
		}else{
			$data['circloid_setting'] = 0;
		}
	...
	public function getLowStock(){
		...
		$low_stock_count = $this->getCircloidSetting('low_stock_count');

		// Get total number of Low Stock items
		$query = $this->db->query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "product WHERE quantity <= '" . $low_stock_count . "'");
	...
	public function saveCircloidSettings($data){

		foreach($data as $settings_name => $settings_value){
			$this->db->query("UPDATE " . DB_PREFIX . "circloid_settings SET value = '" . $this->db->escape($settings_value) . "' WHERE name = '" . $this->db->escape($settings_name) . "'");
		}
	...
	public function installCircloidSettings(){
		...
		$table_exists = $this->tablesExist();

		$file = DIR_APPLICATION . 'base5builder_circloid_settings/update_circloid_settings.sql';
			
		if($table_exists['circloid_setting']){
			if(file_exists($file)){
				unlink($file);
				rmdir(DIR_APPLICATION . 'base5builder_circloid_settings');
			...
			if(file_exists($file)){
				...
				// Remove other past installation files/folders used by Circloid
				$file_previous = DIR_APPLICATION . 'base5builder_circloid_install/install_circloid_color_widget.sql';
				$file_previous_2 = DIR_APPLICATION . 'base5builder_circloid_install/update_circloid_color_widget.sql';
				if(file_exists($file_previous)){
					unlink($file_previous);
					if(file_exists($file_previous_2)){
						unlink($file_previous_2);
						rmdir(DIR_APPLICATION . 'base5builder_circloid_install');
					}
				}
	...
	public function getCircloidSettings(){

		$table_exists = $this->tablesExist();

		if($table_exists['circloid_setting']){
			$query = $this->db->query("SELECT name, value FROM " . DB_PREFIX . "circloid_settings");
	...
	public function getCircloidSetting($setting_name){

		$table_exists = $this->tablesExist();

		if($table_exists['circloid_setting']){
			$query = $this->db->query("SELECT value FROM " . DB_PREFIX . "circloid_settings WHERE name = '" . $setting_name . "'");
