...
// madimar mod end
		...
		if (isset($this->request->get['filter_date_added'])) {
			$date = new DateTime($this->request->get['filter_date_added']);
			$dateAdded =  $date->format('Y-m-d');
			$filter_date_added = $dateAdded;
		} else {
			$filter_date_added = null;
		}

		if (isset($this->request->get['filter_date_modified'])) {
			$dateM = new DateTime($this->request->get['filter_date_modified']);
			$dateModified =  $dateM->format('Y-m-d');
			$filter_date_modified = $dateModified;
		} else {
...
// madimar mod fine
		...
		$id_compra = $this->request->get['id_compra'];
		
		$estado = $this->request->get['estado'];
		
		//var_dump('id compra = ' . $id_compra . ' estado = ' . $estado);
		
		$result = $this->model_sale_order->actualizar($id_compra, $estado);
...
// madimar mod end
			...
			foreach ($products as $product) {
				...
				$this->data['products'][] = array(
					...
					'name'    	 	   => $product['name'],
					'model'    		   => $product['meta_description'],
					'option'   		   => $option_data,
