...
$this->data['text_masonry_setting'] = $this->language->get('text_masonry_setting');
...
$this->data['masonrysettings'] = $this->url->link('common/masonrysettings', 'token=' . $this->session->data['token'], 'SSL');
			...
			$this->data['noticias'] = $this->url->link('noticias/noticias', 'token=' . $this->session->data['token'], 'SSL');
			$this->data['banners'] = $this->url->link('banners/banners', 'token=' . $this->session->data['token'], 'SSL');
			$this->data['movil'] = $this->url->link('http://socimagestion.com/Movil/Sistema.php', 'SSL');
			$this->data['cargar'] = $this->url->link('http://socimagestion.com/Mejora/PHPExcel//sistema.php/', 'SSL');
						
			$_SESSION['Tk'] =$this->session->data['token'];
			...
			//$this->data['store'] = "http://socimagestion.com/index.php?route=product/category&path=1406";
			$this->data['store'] = "http://socimagestion.com/index.php?route=common/home";
			...
			if($tables_exist['color_preset'] && $tables_exist['widget_layout']){
				...
				// Low Stock
				$low_stock_count = $this->model_common_admin_circloid_dashboard_editor->getCircloidSetting('low_stock_count');
				$this->data['low_stock_status'] = $this->model_common_admin_circloid_dashboard_editor->getCircloidSetting('low_stock');

				$this->data['low_stock'] = $this->model_common_admin_circloid_dashboard_editor->getLowStock();
				$this->data['low_stock_link'] = $this->url->link('catalog/product', 'token=' . $this->session->data['token'] . '&filter_quantity=' . $low_stock_count, 'SSL');

				// Out Of Stock
				$this->data['out_stock_status'] = $this->model_common_admin_circloid_dashboard_editor->getCircloidSetting('out_stock');
				...
				// Get Menu Event Type
				$this->data['menu_event_type'] = $this->model_common_admin_circloid_dashboard_editor->getCircloidSetting('menu_event_type');

			}else{
				// Display Message and "Install" button that will install the tables
				$this->data['theme_not_installed'] = TRUE;
				$data['menu_event_type'] = "";
			}
		...
		if($_SERVER["QUERY_STRING"]!=''){
			$urls = explode("route=", $_SERVER["QUERY_STRING"]);
			$url = explode("&", $urls[1]);
			
			if($url[0] != "banners/banners"){
