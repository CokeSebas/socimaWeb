...
class ModelCatalogCategory extends Model {
	...
	public function getCategories2($data) {
		$sql = "SELECT ca.category_id AS category_id, cd.name AS name, ca.parent_id, ca.sort_order
				FROM " . DB_PREFIX . "category ca JOIN " . DB_PREFIX . "category_description cd ON (ca.category_id = cd.category_id)
				JOIN " . DB_PREFIX . "category_path cp ON (cd.category_id = cp.category_id)
				WHERE cp.level != 0 AND cd.language_id = '" . (int)$this->config->get('config_language_id') . "'";
	...
	public function getProduct2($data) {
	
		$sql = "SELECT * FROM ". DB_PREFIX ."product WHERE product_id LIKE '%".$data['filter_name']."%' OR `model` LIKE '%".$data['filter_name']."%'";
				
		$sql .= " GROUP BY product_id ORDER BY model";
