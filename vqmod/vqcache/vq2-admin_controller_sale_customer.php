...
class ControllerSaleCustomer extends Controller { 
			...
			public function getTopCustomers(){
				...
				switch ($range) {
					case 'day':
					$sql = "SELECT SUM(total) AS total, store_id, CONCAT(firstname, ' ', lastname) AS name, customer_id FROM `" . DB_PREFIX . "order` WHERE store_id = '" . $this->config->get("config_store_id") . "' AND order_status_id = '" . (int)$order_status_complete_c . "' AND date_added BETWEEN '" . $beginning_of_today . "' AND '" . $today_date . "' GROUP BY customer_id ORDER BY SUM(total) DESC LIMIT 5";
					...
					if($query->num_rows){
						...
						foreach($rows as $row){
							...
							$data['total_currency'][] = $this->currency->format($row['total'], $this->config->get('config_currency'));
						}
					}else{
						$data['name'][] = "";
						$data['total'][] = "";
					...
					case 'week':
					$sql = "SELECT SUM(total) AS total, store_id, CONCAT(firstname, ' ', lastname) AS name, customer_id FROM `" . DB_PREFIX . "order` WHERE store_id = '" . $this->config->get("config_store_id") . "' AND order_status_id = '" . (int)$order_status_complete_c . "' AND date_added BETWEEN '" . $first_day_of_week . "' AND '" . $today_date . "' GROUP BY customer_id ORDER BY SUM(total) DESC LIMIT 5";
					...
					default:
					case 'month':
					$sql = "SELECT SUM(total) AS total, store_id, CONCAT(firstname, ' ', lastname) AS name, customer_id FROM `" . DB_PREFIX . "order` WHERE store_id = '" . $this->config->get("config_store_id") . "' AND order_status_id = '" . (int)$order_status_complete_c . "' AND date_added BETWEEN '" . $first_day_of_month . "' AND '" . $today_date . "' GROUP BY customer_id ORDER BY SUM(total) DESC LIMIT 5";
					...
					case 'year':
					$sql = "SELECT SUM(total) AS total, store_id, CONCAT(firstname, ' ', lastname) AS name, customer_id FROM `" . DB_PREFIX . "order` WHERE store_id = '" . $this->config->get("config_store_id") . "' AND order_status_id = '" . (int)$order_status_complete_c . "' AND date_added BETWEEN '" . $first_day_of_year . "' AND '" . $today_date . "' GROUP BY customer_id ORDER BY SUM(total) DESC LIMIT 5";
...
// madimar mod end
	...
	protected function getForm() {
		...
		$this->data['column_status'] = $this->language->get('column_status');
		$this->data['column_total2'] = $this->language->get('column_total2');
		$this->data['column_date_added2'] = $this->language->get('column_date_added2');
...
// madimar mod end
		...
		$this->data['entry_credit_total'] = $this->language->get('entry_credito_maximo'); 
		$this->data['entry_credit_actual'] = $this->language->get('entry_credito'); 
		$this->data['entry_vendedor']	 = $this->language->get('entry_vendedor');
...
//var_dump($customer_info);
		...
		if (isset($this->request->post['creditoMaximo'])) {
			$this->data['creditoMaximo'] = $this->request->post['creditoMaximo'];
		} elseif (!empty($customer_info)) { 
			$this->data['creditoMaximo'] = $customer_info['CreditoMaximo'];
		} else {
			$this->data['creditoMaximo'] = '';
		}
		
		if (isset($this->request->post['creditoActual'])) {
			$this->data['creditoActual'] = $this->request->post['creditoActual'];
		} elseif (!empty($customer_info)) { 
			$this->data['creditoActual'] = $customer_info['Credito'];
		} else {
			$this->data['creditoActual'] = '';
		}
...
// madimar mod end
		...
		$vendedores = $this->model_sale_customer->getSalesRep();
		//var_dump($vendedores);
		$this->data['vendedores'] = $vendedores;
		
		if (isset($this->request->post['vendedor'])) {
			$this->data['vendedor'] = $this->request->post['vendedor'];
		} elseif (!empty($customer_info)) {
			$this->data['vendedor'] = $customer_info['salesrep_id'];
		} else {
			$this->data['vendedor'] = '';
		}
...
/////////////////////////////////////////////////////////////////////////
		...
		if(isset($this->request->get['customer_id'])){
			$idCustomer = $this->request->get['customer_id'];
		}else{
			$idCustomer = 0;
		}
		...
		$results = $this->model_sale_customer->getOrders($data);
