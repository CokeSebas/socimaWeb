...
class ControllerCatalogProduct extends Controller {
			...
			public function getTopProducts(){
				...
				switch ($range) {
					case 'day':
					$sql = "SELECT SUM(op.quantity) AS quantity, op.name AS product_name, op.product_id AS product_id FROM `" . DB_PREFIX . "order_product` AS op JOIN `" . DB_PREFIX . "order` AS o ON (op.order_id = o.order_id) WHERE o.store_id = '" . $this->config->get("config_store_id") . "' AND o.order_status_id = '" . (int)$order_status_complete_c . "' AND date_added BETWEEN '" . $beginning_of_today . "' AND '" . $today_date . "' GROUP BY op.name ORDER BY SUM(op.quantity) DESC LIMIT 5";
					...
					case 'week':
					$sql = "SELECT SUM(op.quantity) AS quantity, op.name AS product_name, op.product_id AS product_id FROM `" . DB_PREFIX . "order_product` AS op JOIN `" . DB_PREFIX . "order` AS o ON (op.order_id = o.order_id) WHERE o.store_id = '" . $this->config->get("config_store_id") . "' AND o.order_status_id = '" . (int)$order_status_complete_c . "' AND date_added BETWEEN '" . $first_day_of_week . "' AND '" . $today_date . "' GROUP BY op.name ORDER BY SUM(op.quantity) DESC LIMIT 5";
					...
					default:
					case 'month':
					$sql = "SELECT SUM(op.quantity) AS quantity, op.name AS product_name, op.product_id AS product_id FROM `" . DB_PREFIX . "order_product` AS op JOIN `" . DB_PREFIX . "order` AS o ON (op.order_id = o.order_id) WHERE o.store_id = '" . $this->config->get("config_store_id") . "' AND o.order_status_id = '" . (int)$order_status_complete_c . "' AND date_added BETWEEN '" . $first_day_of_month . "' AND '" . $today_date . "' GROUP BY op.name ORDER BY SUM(op.quantity) DESC LIMIT 5";
					...
					case 'year':
					$sql = "SELECT SUM(op.quantity) AS quantity, op.name AS product_name, op.product_id AS product_id FROM `" . DB_PREFIX . "order_product` AS op JOIN `" . DB_PREFIX . "order` AS o ON (op.order_id = o.order_id) WHERE o.store_id = '" . $this->config->get("config_store_id") . "' AND o.order_status_id = '" . (int)$order_status_complete_c . "' AND date_added BETWEEN '" . $first_day_of_year . "' AND '" . $today_date . "' GROUP BY op.name ORDER BY SUM(op.quantity) DESC LIMIT 5";
...
//var_dump($this->validateForm());
		...
		if (isset($this->request->get['filter_upc'])) {
			...
			/*$imagenes = $this->model_catalog_product->getImages($this->request->get['product_id']);
			foreach($imagenes as $imagens){
				//var_dump($imagens['image']);
				$thumb = $this->model_catalog_product->resize($imagens['image'], 500, 500);
				//var_dump($thumb);
			}*/
...
if (isset($this->request->get['filter_sku'])) {$filter_sku = $this->request->get['filter_sku'];		} else {$filter_sku = null;		}
		if (isset($this->request->get['filter_price'])) {
...
'filter_sku'	  => $filter_sku,
		...
		foreach ($results as $result) {
			...
			if(isset($images[1])){
				if(strlen($images[1]) == 11){
					$imagen = $images[0].'/0'.$images[1];
				}else{
					$imagen = $result['image'];
				}
			}else{
			...
			if ($imagen && file_exists(DIR_IMAGE . $imagen)) {
				$image = $this->model_tool_image->resize($imagen, 40, 40);
			} else {
...
$this->data['column_sku'] = $this->language->get('Codigo');
...
if (isset($this->request->get['filter_sku'])) {$filter_sku = $this->request->get['filter_sku'];			} else {			$filter_sku = '';			}			
