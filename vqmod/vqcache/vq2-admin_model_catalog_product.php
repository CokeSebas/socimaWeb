...
//////Jorge, cache de imagenes
	public function getImages($id_product){
		$query = $this->db->query("SELECT image FROM ".DB_PREFIX."product_image WHERE product_id = " . $id_product . "");
...
////////////Jorge, cache de imagenes
	...
	public function addProduct($data) {
		...
		$imagenes = $this->getImages($product_id);
		foreach($imagenes as $imagens){
			//var_dump($imagens['image']);
			$thumb = $this->resize($imagens['image'], 500, 500);
			//var_dump($thumb);
		}		
	...
	public function editProduct($product_id, $data) {
		...
		$price2 = str_replace("$", '', $data['price']);
		$price = str_replace(".", '', $price2);
		
		$this->db->query("UPDATE " . DB_PREFIX . "product SET model = '" . $this->db->escape($data['model']) . "', sku = '" . $this->db->escape($data['sku']) . "', upc = '" . $this->db->escape($data['upc']) . "', ean = '" . $this->db->escape($data['ean']) . "', jan = '" . $this->db->escape($data['jan']) . "', isbn = '" . $this->db->escape($data['isbn']) . "', mpn = '" . $this->db->escape($data['mpn']) . "', location = '" . $this->db->escape($data['location']) . "', quantity = '" . (int)$data['quantity'] . "', minimum = '" . (int)$data['minimum'] . "', subtract = '" . (int)$data['subtract'] . "', stock_status_id = '" . (int)$data['stock_status_id'] . "', date_available = '" . $this->db->escape($data['date_available']) . "', manufacturer_id = '" . (int)$data['manufacturer_id'] . "', shipping = '" . (int)$data['shipping'] . "', price = '" . (float)$price . "', points = '" . (int)$data['points'] . "', weight = '" . (float)$data['weight'] . "', weight_class_id = '" . (int)$data['weight_class_id'] . "', length = '" . (float)$data['length'] . "', width = '" . (float)$data['width'] . "', height = '" . (float)$data['height'] . "', length_class_id = '" . (int)$data['length_class_id'] . "', status = '" . (int)$data['status'] . "', tax_class_id = '" . $this->db->escape($data['tax_class_id']) . "', sort_order = '" . (int)$data['sort_order'] . "', date_modified = NOW(), modificado ='TRUE' WHERE product_id = '" . (int)$product_id . "'");
	...
	public function getProducts($data = array()) {
		...
		if (!empty($data['filter_name'])) {
			...
			$sql .= " AND pd.name LIKE '" . $this->db->escape($data['filter_name']) . "%' OR pd.product_id LIKE '" . $this->db->escape($data['filter_name']) . "%'";
		}
		...
		if (isset($data['filter_quantity']) && !is_null($data['filter_quantity'])) {
				...
				if($this->db->escape($data['filter_quantity']) <= $low_stock_count){
