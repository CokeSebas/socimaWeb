...
class ModelSaleOrder extends Model {
	...
	public function getVendedores(){
		$query = $this->db->query("SELECT us.*, sl.salesrep_id, IFNULL(SUM(o.total), 0) AS total
									FROM  " . DB_PREFIX . "user us 
									JOIN " . DB_PREFIX . "salesrep sl ON (us.user_id = sl.user_id)
									JOIN " . DB_PREFIX . "order o ON (sl.salesrep_id = o.salesrep_id)
									WHERE  us.user_group_id = 11 AND DATE_FORMAT(o.date_added, '%m-%Y') = DATE_FORMAT(CURDATE(), '%m-%Y')
									GROUP BY us.username");
		return $query->rows;
		
	}
	
	public function getVendedor($user_id){
		$query = $this->db->query("SELECT us.*, sl.salesrep_id, IFNULL(SUM(o.total), 0) AS total
									FROM  " . DB_PREFIX . "user us 
									JOIN " . DB_PREFIX . "salesrep sl ON (us.user_id = sl.user_id)
									JOIN " . DB_PREFIX . "order o ON (sl.salesrep_id = o.salesrep_id)
									WHERE  us.user_id = " . $user_id . " AND DATE_FORMAT(o.date_added, '%m-%Y') = DATE_FORMAT(CURDATE(), '%m-%Y')
									GROUP BY us.username");
	...
	public function getOrders($data = array()) {
		$sql = "SELECT o.order_id, CONCAT(o.firstname, ' ', o.lastname) AS customer, (SELECT os.name FROM " . DB_PREFIX . "order_status os WHERE os.order_status_id = o.order_status_id AND os.language_id = '" . (int)$this->config->get('config_language_id') . "') AS status, o.total, o.currency_code, o.currency_value, o.date_added, o.date_modified, o.salesrep_id AS salesrep FROM `" . DB_PREFIX . "order` o";

		if (isset($data['filter_order_status_id']) && !is_null($data['filter_order_status_id'])) {
			$sql .= " WHERE o.order_status_id = '" . (int)$data['filter_order_status_id'] . "'";
		} else {
			//$sql .= " WHERE o.order_status_id > '0' AND order_status_id != 15";
			$sql .= " WHERE o.order_status_id != 15 AND o.order_status_id != 0";
		}
		...
		if (!empty($data['filter_customer'])) {
			...
			$sql .= " AND CONCAT(o.firstname, ' ', o.lastname) = '" . $this->db->escape($data['filter_customer']) . "'";
		}
...
//var_dump($sql);
	...
	//Jorge
	public function getDatosOrderProcesadas(){
		$sql = "SELECT o.order_id, o.date_added, c.firstname, c.lastname AS rut, a.address_1, a.city, z.name, o.payment_method
				FROM " . DB_PREFIX . "order o JOIN " . DB_PREFIX . "customer c ON (o.customer_id = c.customer_id)
				JOIN " . DB_PREFIX . "address a ON(c.customer_id = a.customer_id)
				JOIN " . DB_PREFIX . "zone z ON (z.zone_id = a.zone_id)
	...
	//Jorge
	public function getOrderProcesadas($order_id){
	...
	public function actualizar($id_compra, $id_estado){
		$this->db->query("UPDATE " . DB_PREFIX . "order SET order_status_id = " . (int)$id_estado . " WHERE order_id = " . (int)$id_compra);
	}
	
	public function getOrderProducts($order_id) {
		$query = $this->db->query("SELECT op.order_product_id, op.order_id, op.product_id, op.name, od.meta_description, op.quantity, op.price, op.total, op.tax, op.reward 
									FROM ".DB_PREFIX."order_product op JOIN ".DB_PREFIX."product_description od ON (op.product_id = od.product_id)
	...
	public function getTotalSales() {
		...
		return $query->row['total'];
				...
				public function getTotalSalesToday() {
					$order_status_complete_c = $this->config->get('config_complete_status_id');

					$query = $this->db->query("SELECT SUM(total) AS total FROM `" . DB_PREFIX . "order` WHERE order_status_id = '" . (int)$order_status_complete_c . "' AND DATE(date_added) = CURDATE()");
				...
				public function getTotalSalesThisWeek() {
					...
					$query = $this->db->query("SELECT SUM(total) AS total FROM `" . DB_PREFIX . "order` WHERE date_added BETWEEN DATE_SUB(CURDATE(), INTERVAL (DAYOFWEEK(CURDATE()) - 1) DAY) AND DATE_ADD(CURDATE(), INTERVAL(8 - DAYOFWEEK(CURDATE())) DAY) AND order_status_id = '" . (int)$order_status_complete_c . "'");
				...
				public function getTotalSalesThisMonth() {
					...
					$query = $this->db->query("SELECT SUM(total) AS total FROM `" . DB_PREFIX . "order` WHERE order_status_id = '" . (int)$order_status_complete_c . "' AND YEAR(date_added) = YEAR(NOW()) AND MONTH(date_added) = MONTH(NOW())");
				...
				public function getAllStatuses() {
					$query = $this->db->query("SELECT order_status_id, name FROM `" . DB_PREFIX . "order_status` WHERE language_id = '" . (int)$this->config->get('config_language_id') . "' ORDER BY name ASC");
	...
	public function getTotalSalesByYear($year) {
			...
			$query = $this->db->query("SELECT SUM(total) AS total FROM `" . DB_PREFIX . "order` WHERE order_status_id = '" . (int)$order_status_complete_c . "' AND YEAR(date_added) = '" . (int)$year . "'");
