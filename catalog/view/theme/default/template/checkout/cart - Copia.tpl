...
<?php echo $content_top; ?>
  ...
  <h1><?php echo $heading_title; ?>
    ...
    &nbsp;(<?php echo $weight; ?>)
    <?php } ?>
      <input id="IDP" type="text" name="ProductoID" value="Código de producto" placeholder="" onFocus="javascript:this.value=''">
      <input id="CTP" type="text" name="Cantidad" value="Cantidad" placeholder="" onFocus="javascript:this.value=''">

      <script type="text/javascript">
          var IDP =  document.getElementById("IDP");
          var CTP =  document.getElementById("CTP");
          var pagina="http://socimagestion.com/index.php?route=checkout/cart"
	  function redireccionar() 
		{
			location.href=pagina
		} 
           
          function ingresar(IDP,CTP)
          {
              addToCart(parseInt(IDP),parseInt(CTP));
	      setTimeout ("redireccionar()", 1000);
          }
        
          document.write('&nbsp; &nbsp;<input type="button" value="Agregar al Carro" onclick="ingresar(IDP.value,CTP.value);" class="button"> ');
  ...
  <form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data">
    <div class="cart-info">
      <table>
        <thead>
          <tr>
           <td class="sku"><?php echo "Codigo"; ?></td>
        ...
        <tbody>
          ...
          <tr>
           <td class="sku"><?php echo $product['sku']; ?></td>
            ...
            <td class="name"><a href="<?php echo $product['href']; ?>"><?php echo $product['name']; ?></a> <?php 
              include('basededatos.php');
	     $vConex =  mysqli_connect($vServer,$vUser,$vPassword,$vBd);
	     
             
	     $vSqlCantidadRecomendada = "select description from oc_product_description where product_id = \"".$product['key']."\" ";
	     $Resultado =  mysqli_query($vConex,$vSqlCantidadRecomendada);
	     $R = "";
	     while($vArreglo = mysqli_fetch_row($Resultado))
	      {
	         echo $R = "<br><span style=\"font-size:.9em\">$vArreglo[0] </span> ";
	       }
           ?>
 	...
 	<tr>
      ...
      <tr>
        ...
        <?php
         ...
         $ValorFinal = (string)($ValorIva3 - $ValorFinal);
         ...
         echo "$" . number_format($ValorFinal2, 0, ',', '.');
         //echo  $ValorFinal2;
...
<tr>
	...
	<td class="right"><?php echo "$" . number_format(round(str_replace(".","",$ValorIva1) * 0.19),0,',','.'); ?></td>	
       ...
       <tr>
 <td class="right"><b><?php echo "Credito Maximo"; ?>:</b></td>
 <?php
       $vSqlCredito= "select Credito from oc_customer where customer_id = \"".$_SESSION['Cienteid'] ."\" ";
	     ...
	     while($vArreglo = mysqli_fetch_row($ResultadoCredito ))
	      {
	         ...
	         {
	        	 $Credito = "Sin Credito";
	        	 echo "<td class=\"right\">".$Credito."</td>";
	     	 }
	     	 else
	     	 {
	     	 	$Credito = $vArreglo[0];
	     	 	...
	     	 	?>
	     	 	<tr><td class="right"><b><?php echo "Credito Disponible"; ?>:</b></td>
                        <?php 
                        echo "<td class=\"right\">$".number_format(round($Credito - $ValorIva3),0,',','.')."</td></tr>";
	     	 }
	      }	  
      ?>
    </table>
  ...
  <div class="buttons">
    ...
    <div class="center"><a href="http://socimagestion.com/index.php?route=product/category&path=1406" class="button"><?php echo $button_shopping; ?></a></div>
  </div>
