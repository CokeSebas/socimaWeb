...
<div id="content"><?php echo $content_top; ?>
  ...
  <?php if (!$logged) { ?>
  <div id="checkout">
  	<a href="http://socimagestion.com/admin/" class="button">Seleccionar Cliente</a>
  </div>
  <?php } ?>
  ...
  <!--crear/ocultar el siguiente div -->
    <?php if ($shipping_required) { ?>
  ...
  <div class="slider-right">
    <div id="confirm">
      <div class="button">CONFIRMAR VENTA</div>
      <div class="onecheckout-content"></div>
    </div>
  </div>
...
function logcreateorder(idOrden){
	$.ajax({
		url: 'index.php?route=onecheckout/address/payment',
		type: 'post',
		data: $('#payment-address input[type=\'text\'], #payment-address input[type=\'password\'], #payment-address input[type=\'checkbox\']:checked, #payment-address input[type=\'radio\']:checked, #payment-address select'),
...
function postshippingmethod(idOrden) {
	$.ajax({
		url: 'index.php?route=onecheckout/shipping',
		type: 'post',
		data: $('#shipping-method input[type=\'radio\']:checked'),
...
function postpaymentmethod(idOrden) {
	$.ajax({
		url: 'index.php?route=onecheckout/payment', 
		type: 'post',
		...
		success: function(json) {
			...
			if (json['error']) {
				...
				//alert('valor orden 4 ' + idOrden);
				paymentbutton(idOrden);
			}
		}
	});	
}

function paymentbutton(idOrden) {
document.getElementById('statusOrder').value = idOrden;
var descuento = $("#dcto").val();
	$.ajax({
		url: 'index.php?route=onecheckout/confirm/createorder'+'&status='+idOrden+'&dcto='+descuento,
		dataType: 'json',
		success: function(json) {
			if (json['redirect']) {
				location = json['redirect'];
	...
	$.ajax({
		...
		success: function(json) {
			...
			if (json['error']) {
				...
				if (json['islogged']) {
					//alert('valor orden ' + idOrden);
					logcreateorder(idOrden);
				} else {
					//alert('Confirmar orden');
					createorder(idOrden);
				}
			}
		}
...
$('#button-penorder').live('click', function() {
var idOrden = '1';
if($('.onecheckout-content .error:first').html()) {
  $('.success, .warning').remove();
  $('#confirm .onecheckout-content').prepend('<div class="warning" style="display: none;">' + $('.onecheckout-content .error:first').html() + '</div>');					
  $('.warning').fadeIn('slow');	
	...
	$.ajax({
		...
		dataType: 'json',
		beforeSend: function() {
			$('#button-penorder').attr('disabled', true);
			$('#button-penorder').after('<span class="wait">&nbsp;<img src="catalog/view/theme/default/image/loading.gif" alt="" /></span>');
		},		
		success: function(json) {
			$('.success, .warning').remove();	
