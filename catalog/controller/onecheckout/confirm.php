...
//var_dump($_SESSION);
			//$data['shipping_method'] = 'Retirar en Socima';
	...
	public function createorder(){
		...
		if (!$json) {
			...
			if (isset($this->session->data['vouchers']) && is_array($this->session->data['vouchers'])) {
				...
				$user = $this->model_checkout_order->salesrep($_SESSION['user_id']);
				//var_dump($user['salesrep_id']);
				
				$data['user_id'] = $_SESSION['user_id'];
				//$data['user_id'] = $user['salesrep_id'];
						
				$localidad = $this->model_checkout_order->get_localidad($_SESSION['user_id']);
				if(isset($localidad['localidad'])){
					if($localidad['localidad'] == 'STGO'){			
						...
						$Titulo = "Nuevo Pedido Santiago del Vendedor " . $localidad['name'] . ' - ' .  $this->session->data['order_id'];
						$Mensaje = "<p>Estimados</p> Se ha procesado el siguiente pedido adjunto:  http://socimagestion.com/admin/view/template/sale/excel_orders2.php?orderid=".$this->session->data['order_id'];
						$Nombre = "Nuevo Pedido Santiago del Vendedor " . $localidad['name'] . ' - ' .  $this->session->data['order_id'];
						...
						/*$cliente = $this->model_checkout_order->get_cliente($_SESSION['customer_id']);
						if($cliente['email'] != ''){
							$correoCliente = $cliente['email'];
						}else{
							$correoCliente = "clientes@socimagestion.com";
						}*/
						$correoCliente = "clientes@socimagestion.com, odril.jtapia@gmail.com ";
						$tituloCliente = "Nueva orden Ingresada";
						$mensajeCliente = "Se a registrado el siguiente pedido : http://socimagestion.com/Movil/order_info.php?order_id=".$this->session->data['order_id'];
					...
					}else if($localidad['localidad'] == 'REG'){
						$this->session->data['order_id'] = $this->model_checkout_order->addOrder($data);			

						//$Correo = "cokesebas@gmail.com";
						$Correo = "rodrigoarenas9@gmail.com, cokesebas@gmail.com, region@socimagestion.com";
						$Titulo = "Nuevo Pedido Region del Vendedor " . $localidad['name'] . ' - ' .  $this->session->data['order_id'];
						$Mensaje = "<p>Estimados</p> Se ha procesado el siguiente pedido adjunto:  http://socimagestion.com/admin/view/template/sale/excel_orders2.php?orderid=".$this->session->data['order_id'];
						$Nombre = "Nuevo Pedido Region del Vendedor " . $localidad['name'] . ' - ' .  $this->session->data['order_id'];
						$mailer = new AttachMailer("sistema@socimagestion.com", $Correo, $Titulo, "<p>".$Mensaje."</p>  El correo se ha generado el  : ".date("d-m-Y"));
						$mailer->send() ? "Enviado": "Problema al enviar";
					...
					$Correo = "cokesebas@gmail.com";
					//$Correo = "rodrigoarenas9@gmail.com, cokesebas@gmail.com, region@socimagestion.com";
					$Titulo = "Nuevo Pedido Region del Vendedor  - " .  $this->session->data['order_id'];
					$Mensaje = "<p>Estimados</p> Se ha procesado el siguiente pedido adjunto:  http://socimagestion.com/admin/view/template/sale/excel_orders2.php?orderid=".$this->session->data['order_id'];
					$Nombre = "Nuevo Pedido Region del Vendedor  - " .  $this->session->data['order_id'];
					$mailer = new AttachMailer("sistema@socimagestion.com", $Correo, $Titulo, "<p>".$Mensaje."</p>  El correo se ha generado el  : ".date("d-m-Y"));
					$mailer->send() ? "Enviado": "Problema al enviar";
				...
				if($_GET['status'] == '1'){
					//var_dump('orden pendiente');
					$this->data['payment'] = $this->getChild('payment/bank_transfer2');
					//var_Dump($this->data['payment'] = $this->getChild('payment/bank_transfer2'));
					$this->data['button_back'] = $this->language->get('button_back');
				}else{
					//var_dump('payment/' . $this->session->data['payment_method']['code']);
					$this->data['payment'] = $this->getChild('payment/' . $this->session->data['payment_method']['code']);
					//var_Dump($this->data['payment'] = $this->getChild('payment/' . $this->session->data['payment_method']['code']));
					$this->data['button_back'] = $this->language->get('button_back');
				}
