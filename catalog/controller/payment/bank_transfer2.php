...
class ControllerPaymentBankTransfer2 extends Controller {
	protected function index() {
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/bank_transfer2.tpl')) {
			$this->template = $this->config->get('config_template') . '/template/payment/bank_transfer2.tpl';
		} else {
	...
	public function confirm() {
		...
		$this->model_checkout_order->confirm($this->session->data['order_id'], 1, $comment, true);
	}
}
?>